...
import { SparklesIcon, BotIcon, InboxInIcon, CheckCircleIcon, Undo2Icon, MessageSquareIcon, SendIcon, UserIcon, ShieldCheckIcon, BarChart3Icon, LightbulbIcon } from './icons';
...
export default function LetterGenerator() {
    ...
    const handleGenerate = async () => {
        if (!objectiveText.trim()) return toast.error("يرجى تحديد غرض الخطاب.");
        setIsLoading(true);
        try {
            const selectedPath = strategyAnalysis?.paths.find(p => p.id === selectedPathId);
            const result = await generateLetterVariations({
                isReply: isReplyMode,
                originalContent: originalLetterContent,
                objective: objectiveText,
                sender, receiver, subject,
                strategyLogic: selectedPath?.logic
            });
    ...
    const handleChatRefine = async () => {
        ...
        try {
            ...
            setChatMessages(prev => [...prev, {role: 'ai', text: 'تم تحديث النص بناءً على طلبك.'}]);
    ...
    const handleFinalSave = () => {
        ...
        toast.success("تم اعتماد الخطاب وحفظه.");
    };
    ...
    return (
        <div className="max-w-[1700px] mx-auto pb-12">
            ...
            <div className="flex justify-between items-end mb-8 px-2">
                <div>
                    ...
                    <p className="text-slate-400 font-bold mt-2">
                        {isReplyMode ? "تحليل النوايا وبناء الردود الاستراتيجية المحكمة" : "صياغة خطابات رسمية عالية الجودة"}
                    </p>
                </div>
                {step > 0 && (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                {/* Side Panel: Reference & Strategic Insights */}
                <div className="lg:col-span-4 space-y-6">
                    {isReplyMode && parentLetter && (
                        <div className="glass-card border-indigo-500/20 p-6 rounded-3xl space-y-4">
                            ...
                            <div className="text-[11px] text-slate-500 leading-relaxed max-h-32 overflow-y-auto custom-scrollbar opacity-60" dangerouslySetInnerHTML={{ __html: parentLetter.body }} />
                        </div>
                    )}

                    {strategyAnalysis && (
                        <div className="bg-indigo-600/5 border border-indigo-500/10 p-6 rounded-3xl space-y-6 animate-in fade-in duration-700">
                             <div className="space-y-2">
                                <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">فك شفرة النوايا</p>
                                <p className="text-sm font-bold text-slate-200 leading-relaxed italic">"{strategyAnalysis.intent}"</p>
                             </div>
                             <div className="p-4 bg-black/40 rounded-2xl border border-white/5 flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs font-black text-white">{strategyAnalysis.powerBalance}</p>
                                </div>
                             </div>
                             {strategyAnalysis.risks.length > 0 && (
                                 <div className="space-y-3">
                                    <p className="text-[10px] font-black text-rose-500 uppercase tracking-widest">تنبيه المخاطر الإدارية</p>
                                    <ul className="space-y-1.5">
                                        {strategyAnalysis.risks.map((r, i) => (
                                            <li key={i} className="text-[11px] text-slate-400 font-bold flex items-start gap-2">
                                                <span className="mt-1.5 w-1 h-1 bg-rose-500 rounded-full"></span> {r}
                                            </li>
                                        ))}
                                    </ul>
                                 </div>
                            ...
                            <div className="glass-card p-8 rounded-[2.5rem] border-white/10 space-y-8 shadow-3xl">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <textarea 
                                        ...
                                        placeholder="ما الذي تريد تحقيقه من هذا الرد؟"
                                    />
                                </div>
                                ...
                                <div className="flex justify-center">
                                    ...
                                    >
                                        ...
                                        <span>{isLoading ? 'جاري الصياغة والتدقيق...' : 'توليد المسودات الذكية'}</span>
                                    </button>
                                </div>
                            </div>
                        </div>
                    ...
                    {step === 1 && generatedContent && (
                        <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
                             <div className="bg-indigo-500/10 border border-indigo-500/20 p-6 rounded-3xl flex items-start gap-4 shadow-2xl">
                                ...
                                <div>
                                    <p className="text-[10px] font-black text-indigo-400 uppercase mb-1">رؤية الخبير الإداري</p>
                                ...
                                {(['neutral', 'strict', 'diplomatic'] as const).map(vKey => (
                                    <div key={vKey} className="bg-slate-900/80 rounded-[2.2rem] border border-white/5 overflow-hidden flex flex-col hover:border-indigo-500/50 transition-all shadow-xl hover:-translate-y-1">
                                        <div className="p-4 bg-white/5 border-b border-white/10 text-center font-black text-[10px] uppercase text-slate-400 tracking-widest">
                                            {vKey === 'neutral' ? 'صيغة متزنة' : vKey === 'strict' ? 'صيغة حازمة' : 'صيغة دبلوماسية'}
                                        </div>
                                        ...
                                        <div className="p-4"><button onClick={() => { setFinalBody(generatedContent.variations[vKey]); setStep(2); }} className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 rounded-xl font-black text-xs text-white">اعتماد هذه النسخة</button></div>
                                    </div>
                                ))}
                             </div>
                        </div>
                             ...
                             <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
                                ...
                                <div className="xl:col-span-4 flex flex-col h-[700px] glass-card border-white/5 p-6 rounded-[2rem]">
                                    ...
                                    <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4 mb-4 px-2">
                                        {chatMessages.length === 0 && <p className="text-center text-slate-500 text-xs py-20 font-bold">يمكنك طلب تعديلات محددة على النص أعلاه بالحوار...</p>}
                                    ...
                                    <div className="relative mt-auto">
                                        <textarea 
                                            ...
                                            rows={3} 
                                        />
                                        <button onClick={handleChatRefine} disabled={isLoading || !userChatInput.trim()} className="absolute bottom-4 right-3 p-2 bg-indigo-600 text-white rounded-xl shadow-lg disabled:opacity-50 transition-all hover:bg-indigo-500"><SendIcon className="w-4 h-4 rotate-180" /></button>
                                    </div>
                                </div>
                             </div>
                        </div>
