...
export default function LetterGenerator() {
    ...
    // تفعيل التحليل الاستراتيجي الفوري عند البدء في رد
    useEffect(() => {
        if (isReplyMode && parentLetter && !strategyAnalysis && step === 0) {
            ...
            analyzeStrategicPaths(parentLetter)
                ...
                .catch(e => {
                    console.error(e);
                    toast.error("تعذر إجراء التحليل الاستراتيجي حالياً.");
                })
    ...
    const handleGenerate = async () => {
        if (!objectiveText.trim()) return toast.error("يرجى تحديد هدف الخطاب.");
        setIsLoading(true);
        try {
    ...
    const handleChatRefine = async () => {
        ...
        try {
            ...
            setChatMessages(prev => [...prev, {role: 'ai', text: 'تم تعديل النص بناءً على ملاحظاتك.'}]);
        } catch (e) { toast.error("فشل التحديث عبر الحوار."); } finally { setIsLoading(false); }
    };
    ...
    const handleFinalSave = () => {
        ...
        toast.success("تم اعتماد المعاملة وحفظها.");
    };
            ...
            <div className="flex justify-between items-end mb-8 px-2">
                <div>
                    <h2 className="text-3xl font-black text-white tracking-tight flex items-center gap-3">
                        ...
                        {isReplyMode ? "مختبر المناورات الإدارية" : "مركز الصياغة الذكية"}
                    </h2>
                    <p className="text-slate-400 font-bold mt-2">
                        {isReplyMode ? "تحليل موازين القوة وبناء ردود استراتيجية محكمة" : "صياغة خطابات رسمية عالية الجودة"}
                    </p>
                </div>
                {step > 0 && (
                        ...
                        <div className="bg-indigo-600/5 border border-indigo-500/10 p-6 rounded-3xl space-y-6 animate-in fade-in duration-700 shadow-xl">
                             ...
                             {strategyAnalysis.risks.length > 0 && (
                                 <div className="space-y-3">
                                    <p className="text-[10px] font-black text-rose-500 uppercase tracking-widest px-1">تنبيه المخاطر الإدارية</p>
                                    <ul className="space-y-2">
                                        {strategyAnalysis.risks.map((r, i) => (
                                            <li key={i} className="text-[11px] text-slate-400 font-bold flex items-start gap-2">
                                                <span className="mt-1.5 w-1 h-1 bg-rose-500 rounded-full shrink-0"></span> {r}
                                            </li>
                                        ))}
                                    </ul>
                                 </div>
                        ...
                        <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        {strategyAnalysis.paths.map((path) => (
                                            <button 
                                                ...
                                                className={`p-5 rounded-[2.2rem] border text-right transition-all group flex flex-col justify-between min-h-[220px] shadow-2xl ${selectedPathId === path.id ? 'bg-indigo-600/20 border-indigo-500 ring-2 ring-indigo-500/20' : 'bg-slate-900/40 border-white/5 hover:border-indigo-500/30'}`}
                                            >
                                                <div>
                                                    ...
                                                    <p className="text-[10px] text-slate-500 font-medium italic opacity-0 group-hover:opacity-100 transition-all">"{path.logic}"</p>
                                                </div>
                                                <div className="mt-4 pt-3 border-t border-white/5 text-[9px] font-black text-slate-500 group-hover:text-indigo-300 transition-colors">
                                                    {selectedPathId === path.id ? "المسار مفعل ✓" : "تفعيل المسار الاستراتيجي ←"}
                                                </div>
                                            </button>
                                        ))}
                                    </div>
                            ...
                            <div className="glass-card p-8 rounded-[2.5rem] border-white/10 space-y-8 shadow-3xl">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-black/20 p-6 rounded-3xl border border-white/5">
                                    ...
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">إلى (جهة المستلم)</label>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <textarea 
                                        ...
                                        placeholder="اكتب النقاط الرئيسية التي ترغب في طرحها بالخطاب..."
                                    />
                                </div>
                                ...
                                <div className="flex justify-center">
                                    <button 
                                        ...
                                        className={`px-20 py-5 rounded-2xl font-black text-lg flex items-center gap-4 transition-all shadow-2xl active:scale-95 ${isLoading ? 'bg-slate-700 opacity-50 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-500 text-white shadow-indigo-600/30'}`}
                                    >
                             ...
                             <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                {(['neutral', 'strict', 'diplomatic'] as const).map(vKey => (
                                    <div key={vKey} className="bg-slate-900/80 rounded-[2.2rem] border border-white/5 overflow-hidden flex flex-col hover:border-indigo-500/50 transition-all shadow-xl hover:-translate-y-1 group">
                                        ...
                                        <div className="p-4"><button onClick={() => { setFinalBody(generatedContent.variations[vKey]); setStep(2); }} className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 rounded-xl font-black text-xs text-white shadow-lg">اعتماد هذه النسخة للتنقيح</button></div>
                                    </div>
                                ))}
                             </div>
                        </div>
                             ...
                             <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
                                ...
                                <div className="xl:col-span-4 flex flex-col h-[750px] glass-card border-white/5 p-6 rounded-[2.5rem] shadow-2xl">
                                    <div className="flex items-center gap-3 mb-6 border-b border-white/5 pb-4">
                                        ...
                                        <h3 className="text-sm font-black text-white">منصة التنقيح بالحوار</h3>
                                    </div>
                                    ...
                                    <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4 mb-4 px-2">
                                        {chatMessages.length === 0 && (
                                            <div className="h-full flex flex-col items-center justify-center opacity-30 text-center px-4">
                                                ...
                                                <p className="text-xs font-bold leading-relaxed">أنا جاهز لتعديل النص حسب رغبتك. أخبرني بما تريد تغييره أو إضافته.</p>
                                            </div>
                                        )}
