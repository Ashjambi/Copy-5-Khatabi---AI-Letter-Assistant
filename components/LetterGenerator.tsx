...
import { SparklesIcon, BotIcon, InboxInIcon, CheckCircleIcon, Undo2Icon, MessageSquareIcon, SendIcon, UserIcon, ShieldCheckIcon, BarChart3Icon, LayoutTemplateIcon } from './icons';

export default function LetterGenerator() {
    const { state, dispatch } = useApp();
    const { generatorState, companySettings, letters } = state;
    ...
    // تفعيل التحليل الاستراتيجي فور الدخول في نمط الرد
    useEffect(() => {
        if (isReplyMode && parentLetter && !strategicData && step === 0) {
            setIsLoading(true);
            analyzeStrategicPaths(parentLetter)
                .then(data => {
                    setStrategicData(data);
                    toast.success("اكتمل التحليل الاستراتيجي للموقف.");
                })
                .catch(e => {
                    if (e.message.includes('429')) toast.error("تم تجاوز الكوتا اليومية للذكاء الاصطناعي.");
                    else toast.error("فشل التحليل الاستراتيجي.");
                })
    ...
    const handleGenerate = async () => {
        if (!objectiveText.trim()) return toast.error("الرجاء إدخال توجيهات الصياغة أو اختيار مسار.");
        setIsLoading(true);
        try {
            const strategy = strategicData?.suggested_strategies.find(s => s.id === selectedStrategyId);
            ...
            toast.error(e.message || "حدث خطأ أثناء الصياغة.");
        } finally { setIsLoading(false); }
    };
    ...
    return (
        <div className="max-w-[1700px] mx-auto pb-12">
            <div className="flex justify-between items-end mb-8 px-2">
                <div>
                    <h2 className="text-3xl font-black text-white tracking-tight flex items-center gap-3">
                        {isReplyMode ? <BotIcon className="w-8 h-8 text-emerald-400" /> : <SparklesIcon className="w-8 h-8 text-indigo-400" />}
                        {isReplyMode ? "مختبر الاستراتيجيات والردود" : "مركز الصياغة الإدارية"}
                    </h2>
                    <p className="text-slate-400 font-bold mt-2">
                        {isReplyMode ? "نظام الرد الذكي المعتمد على تحليل ميزان القوة الإداري" : "صياغة خطابات رسمية بمعايير بروتوكولية عالية"}
                    </p>
                </div>
                {step > 0 && (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                {/* Side Panel: Context & Strategic Summary */}
                <div className="lg:col-span-4 lg:sticky lg:top-6 space-y-6">
                    {isReplyMode && parentLetter && (
                        <div className="glass-card border-white/5 bg-slate-950/40 p-6 rounded-3xl space-y-4">
                            <div className="flex items-center gap-2 text-[10px] font-black text-emerald-400 uppercase tracking-widest">
                                <InboxInIcon className="w-4 h-4" /> المعاملة الواردة الأصلية
                            </div>
                            <h3 className="text-base font-black text-white">{parentLetter.subject}</h3>
                            <div className="text-[11px] text-slate-500 leading-relaxed max-h-32 overflow-y-auto custom-scrollbar" dangerouslySetInnerHTML={{ __html: parentLetter.body }} />
                        </div>
                    )}
                    ...
                    {strategicData && (
                        <div className="bg-indigo-500/5 border border-indigo-500/10 p-6 rounded-3xl space-y-6 animate-in fade-in duration-500">
                             <div className="space-y-1">
                                <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">تحليل ذكاء الأعمال</p>
                                <p className="text-sm font-bold text-slate-200 leading-relaxed">{strategicData.sender_vibe}</p>
                             </div>
                             <div className="p-4 bg-black/40 rounded-2xl border border-white/5 flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs font-black text-white">
                                        {strategicData.power_dynamic === 'superior' ? 'المرسل في مركز قوة' : strategicData.power_dynamic === 'equal' ? 'ندية إدارية' : 'أفضلية إدارية للمستلم'}
                                    </p>
                                </div>
                             </div>
                             <div>
                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">النقاط الحرجة في الخطاب:</p>
                                <ul className="space-y-2">
                                    {strategicData.critical_points.map((p, i) => (
                        ...
                        <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
                            {isReplyMode && strategicData && (
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        {strategicData.suggested_strategies.map((s) => (
                                            <button 
                                                key={s.id}
                                                onClick={() => { setSelectedStrategyId(s.id); setObjectiveText(s.objective); }}
                                                className={`p-5 rounded-[2.2rem] border text-right transition-all group flex flex-col justify-between min-h-[220px] shadow-2xl ${selectedStrategyId === s.id ? 'bg-indigo-600/20 border-indigo-500 ring-2 ring-indigo-500/20' : 'bg-slate-900/40 border-white/5 hover:border-indigo-500/30'}`}
                                            >
                                                <div>
                                                    <span className={`text-[10px] font-black uppercase mb-3 block ${selectedStrategyId === s.id ? 'text-indigo-400' : 'text-slate-500'}`}>{s.title}</span>
                                                    <p className="text-xs font-black text-slate-100 leading-relaxed mb-4">{s.impact}</p>
                                                    <p className="text-[10px] text-slate-500 leading-relaxed font-medium italic opacity-0 group-hover:opacity-100 transition-opacity">"{s.logic}"</p>
                                                </div>
                                                <div className="mt-4 pt-3 border-t border-white/5 text-[9px] font-black text-slate-500 group-hover:text-indigo-300">
                                                    {selectedStrategyId === s.id ? "المسار مفعل ✓" : "تفعيل الاستراتيجية ←"}
                                                </div>
                                            </button>
                                        ))}
                                    </div>
                            ...
                            <div className="glass-card p-8 rounded-[2.5rem] border-white/10 space-y-8 shadow-3xl">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-black/20 p-6 rounded-3xl">
                                    ...
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">إلى (المستهدف بالخطاب)</label>
                                ...
                                <div className="space-y-4">
                                    <label className="text-sm font-black text-white flex items-center gap-2">
                                        <MessageSquareIcon className="w-5 h-5 text-indigo-400" /> توجيهاتك للصياغة النهائية
                                    </label>
                                    <textarea 
                                        value={objectiveText} 
                                        ...
                                        placeholder="اكتب النقاط الرئيسية التي ترغب في طرحها..."
                                    />
                                </div>
                                ...
                                <div className="flex justify-center">
                                    <button 
                                        ...
                                        className={`px-16 py-5 rounded-2xl font-black text-lg flex items-center gap-4 transition-all shadow-2xl active:scale-95 ${isLoading ? 'bg-slate-700 opacity-50 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-500 text-white'}`}
                                    >
                                        ...
                                        <span>{isLoading ? 'جاري التحليل والصياغة...' : 'توليد المسودات الاستراتيجية'}</span>
                                    </button>
                                </div>
                            </div>
                        </div>
                             ...
                             <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                {(['neutral', 'strict', 'diplomatic'] as const).map(vKey => (
                                    <div key={vKey} className="bg-slate-900/80 rounded-[2.2rem] border border-white/5 overflow-hidden flex flex-col hover:border-indigo-500/50 transition-all shadow-xl hover:-translate-y-1">
                                        <div className="p-4 bg-white/5 border-b border-white/10 text-center font-black text-[10px] uppercase text-slate-400 tracking-widest">
                                            {vKey === 'neutral' ? 'الصيغة الرسمية' : vKey === 'strict' ? 'الصيغة السيادية' : 'الصيغة الدبلوماسية'}
                                        </div>
                                        <div className="p-6 flex-grow text-white text-[13px] leading-relaxed font-bold overflow-y-auto max-h-[350px] text-right prose prose-invert prose-sm" dangerouslySetInnerHTML={{ __html: sanitizeHTML(generatedContent.variations[vKey]) }} />
                                        <div className="p-4"><button onClick={() => { setFinalBody(generatedContent.variations[vKey]); setStep(2); }} className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 rounded-xl font-black text-xs text-white shadow-lg transition-colors">اعتماد هذه النسخة</button></div>
                                    </div>
                                ))}
                             </div>
                        </div>
                        ...
                        <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
                             <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
                                <div className="xl:col-span-8 space-y-6">
                                    <div className="rounded-[2rem] overflow-hidden border border-white/10 shadow-3xl bg-white">
                                    ...
                                    <div className="flex justify-end gap-4">
                                        <button onClick={handleFinalSave} className="px-12 py-4 bg-emerald-600 hover:bg-emerald-500 text-white rounded-2xl font-black flex items-center gap-3 shadow-xl active:scale-95">
                                            <CheckCircleIcon className="w-6 h-6" /> حفظ الخطاب المعتمد
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div className="xl:col-span-4 flex flex-col h-[700px] glass-card border-white/5 p-6 rounded-[2rem]">
                                    <div className="flex items-center gap-3 mb-6">
                                        ...
                                        <h3 className="text-sm font-black text-white">تنقيح بالحوار الذكي</h3>
                                    </div>
                                    ...
                                    <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4 mb-4 px-2">
                                        {chatMessages.length === 0 && (
                                            <div className="h-full flex flex-col items-center justify-center opacity-30 text-center">
                                                ...
                                                <p className="text-xs font-bold">يمكنك طلب تعديلات محددة على النص أعلاه آلياً...</p>
                                            </div>
                                        )}
                                        {chatMessages.map((msg, i) => (
                                            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-start' : 'justify-end'}`}>
                                                <div className={`max-w-[85%] p-4 rounded-2xl text-xs font-bold ${msg.role === 'user' ? 'bg-indigo-600/20 text-indigo-100 border border-indigo-500/20' : 'bg-slate-800 text-slate-200 border border-white/5'}`}>
                                                    {msg.text}
                                                </div>
                                            </div>
                                        ))}
                                    ...
                                    <div className="relative">
                                        <textarea 
                                            ...
                                            className="w-full bg-slate-950/60 border border-white/10 rounded-2xl p-4 text-xs font-bold text-white focus:ring-1 focus:ring-indigo-500 resize-none pr-12 shadow-inner" 
                                            rows={3} 
                                        />
                                        <button 
                                            onClick={handleChatRefine} 
                                            disabled={isLoading || !userChatInput.trim()} 
                                            className="absolute bottom-4 right-3 p-2 bg-indigo-600 text-white rounded-xl shadow-lg disabled:opacity-50 transition-all hover:bg-indigo-500"
                                        >
