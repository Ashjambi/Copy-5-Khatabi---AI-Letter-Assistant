...
/**
 * قاعدة ذهبية: تهيئة العميل داخل كل دالة لضمان الوصول للمفتاح الحقن
 */
const getAIClient = () => {
    return new GoogleGenAI({ apiKey: process.env.API_KEY });
};

const ARABIC_STRICT_RULE = "يجب أن تكون جميع النصوص العربية بكلمات متصلة تماماً وطبيعية (مثلاً: 'الموضوع' وليس 'ا ل م و ض و ع'). يُمنع منعاً باتاً تقطيع الحروف.";

/**
 * المحرك الاستراتيجي: تحليل الموقف الإداري بعمق
 */
    ...
    const response = await ai.models.generateContent({
        ...
        config: {
            ...
            responseSchema: {
                type: Type.OBJECT,
                properties: {
                    sender_vibe: { type: Type.STRING, description: "نبرة المرسل الخفية" },
                    power_dynamic: { type: Type.STRING, enum: ['superior', 'equal', 'subordinate'], description: "ميزان القوة" },
...
/**
 * توليد النسخ الثلاث بناءً على المسار المختار
 */
export async function generateLetterVariations(params: {
    ...
    const response = await ai.models.generateContent({
        model: "gemini-3-pro-preview", // استخدام برو لأعلى جودة في الصياغة
        contents: prompt,
        config: {
            systemInstruction,
            responseMimeType: "application/json",
...
export async function refineLetterWithChat(currentBody: string, userInstruction: string, context: string): Promise<string> {
    ...
    const response = await ai.models.generateContent({
        ...
        config: { 
            systemInstruction: `أنت خبير تنقيح لغوي. عدل النص وأعده بتنسيق HTML. ${ARABIC_STRICT_RULE}` 
        }
    });
...
export async function extractDetailsFromLetterImage(
  ...
  const response = await ai.models.generateContent({
    model: "gemini-3-flash-preview",
    contents: [
        { parts: [{ text: `استخرج البيانات من الصورة المرفقة بصيغة JSON. ${ARABIC_STRICT_RULE}` }, { inlineData: { data: base64Data, mimeType: mimeType || "image/jpeg" } }] }
    ],
    config: {
        responseMimeType: "application/json",
        responseSchema: {
...
export async function searchLettersSmartly(query: string, letters: Letter[]): Promise<any[]> {
    ...
    const response = await ai.models.generateContent({
        model: "gemini-3-flash-preview",
        contents: `ابحث عن ${query} سياقياً في المعاملات.`
    });
    return [];
}
